...
test('rules.date should return "date.past" when date is not in past', (t) => {
  const inFuture = moment(new Date()).add(1, 'days').toISOString();
  t.is(date('field', inFuture, { past: true }), 'date.past');
  t.is(date('field', moment().toISOString(), { past: true }), 'date.past');
});

test('rules.date should return "date.future" when date is not in future', (t) => {
  const inPast = moment(new Date()).subtract(1, 'days').toISOString();
  t.is(date('field', inPast, { future: true }), 'date.future');
  t.is(date('field', moment().toISOString(), { future: true }), 'date.future');
});

test('rules.date should return null when value is equal', (t) => {
  t.is(date('field', moment().toISOString(), { min: moment().format('YYYY-MM-DD') }), null);
  t.is(date('field', moment().toISOString(), { max: moment().format('YYYY-MM-DD') }), null);
});

test('rules.date should return "date.min" when value is lower', (t) => {
  const inPast = moment().subtract(2, 'days').toISOString();
...
test('rules.date should return "date.max" when value is bigger', (t) => {
  const inFuture = moment().add(2, 'days').toISOString();
